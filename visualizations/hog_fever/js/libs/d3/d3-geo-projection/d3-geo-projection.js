...
function hyperellipticalRaw(alpha, k, gamma) {
  ...
  var G = 1 / z(1),
      ...
      m = (1 + 1e-8) * G,
      approx = [];
  for (var i = 0; i <= n;="" i++)="" approx.push(z(i="" n)="" *="" m);="" function="" y(sinphi)="" {="" var="" rmin="0," rmax="n," r="n">> 1;
    do {
...
function interrupt(project, lobes) {
  ...
  if (project.invert) forward.invert = function(x, y) {
    ...
    for (var i = 0, n = bound.length; i < n; ++i) {
      var b = bound[i];
      if (b[0][0] <= x="" &&="" <="" b[1][0]="" b[0][1]="" y="" b[1][1])="" {="" var="" p="project.invert(x" -="" project(lobe[i][1][0],="" 0)[0],="" y);="" p[0]="" +="lobe[i][1][0];" return="" pointequal(forward(p[0],="" p[1]),="" [x,="" y])="" ?="" :="" null;="" }="" };="" stream_="p.stream;" p.stream="function(stream)" rotate="p.rotate()," rotatestream="stream_(stream)," spherestream="(p.rotate([0," 0]),="" stream_(stream));="" p.rotate(rotate);="" rotatestream.sphere="function()" d3geo.geostream(sphere,="" spherestream);="" rotatestream;="" p.lobes="function(_)" if="" (!arguments.length)="" lobes.map(function(lobe)="" lobe.map(function(l)="" [="" [l[0][0]="" *="" degrees,="" l[0][1]="" degrees],="" [l[1][0]="" l[1][1]="" [l[2][0]="" l[2][1]="" degrees]="" ];="" });="" sphere="interpolateSphere(_);" lobes="_.map(function(lobe)" radians,="" radians],="" radians]="" bounds="lobes.map(function(lobe)" x0="project(l[0][0]," l[0][1])[0],="" x1="project(l[2][0]," l[2][1])[0],="" y0="project(l[1][0]," l[0][1])[1],="" y1="project(l[1][0]," l[1][1])[1],="" t;="" (y0=""> y1) t = y0, y0 = y1, y1 = t;
        return [[x0, y0], [x1, y1]];
      });
    });
...
function clockwise(ring) {
  ...
  return area <= 0;="" }="" function="" contains(ring,="" point)="" {="" var="" x="point[0]," y="point[1]," contains="false;" for="" (var="" i="0," n="ring.length," j="n" -="" 1;="" <="" n;="" pi="ring[i]," xi="pi[0]," yi="pi[1]," pj="ring[j]," xj="pj[0]," yj="pj[1];" if="" (((yi=""> y) ^ (yj > y)) && (x < (xj - xi) * (y - yi) / (yj - yi) + xi)) contains = !contains;
  }
  return contains;
}
...
function quantize(input, digits) {
  if (!(0 <= 2="" (digits="+digits)" &&="" digits="" <="20))" throw="" new="" error("invalid="" digits");="" function="" quantizepoint(input)="" {="" var="" n="input.length," i="2," output="new" array(n);="" output[0]="+input[0].toFixed(digits);" output[1]="+input[1].toFixed(digits);" while="" (i="" n)="" output[i]="input[i]," ++i;="" return="" output;="" }="" quantizepoints(input)="" input.map(quantizepoint);="" quantizepolygon(input)="" input.map(quantizepoints);="" quantizegeometry(input)="" if="" (input="=" null)="" input;="" switch="" (input.type)="" case="" "geometrycollection":="" "geometrycollection",="" geometries:="" input.geometries.map(quantizegeometry)};="" break;="" "point":="" "point",="" coordinates:="" quantizepoint(input.coordinates)};="" "multipoint":="" "linestring":="" input.type,="" quantizepoints(input.coordinates)};="" "multilinestring":="" "polygon":="" quantizepolygon(input.coordinates)};="" "multipolygon":="" "multipolygon",="" input.coordinates.map(quantizepolygon)};="" default:="" (input.bbox="" !="null)" output.bbox="input.bbox;" quantizefeature(input)="" "feature",="" properties:="" input.properties,="" geometry:="" quantizegeometry(input.geometry)};="" (input.id="" output.id="input.id;" "feature":="" quantizefeature(input);="" "featurecollection":="" "featurecollection",="" features:="" input.features.map(quantizefeature)};="" quantizegeometry(input);="" rectangularpolyconicraw(phi0)="" sinphi0="sin(phi0);" forward(lambda,="" phi)="" a="sinPhi0" ?="" tan(lambda="" *="" 2)="" :="" lambda="" 2;="" (!phi)="" [2="" a,="" -phi0];="" e="2" atan(a="" sin(phi)),="" cotphi="1" tan(phi);="" [="" sin(e)="" cotphi,="" phi="" +="" (1="" -="" cos(e))="" phi0="" ];="" todo="" null="" for="" points="" outside="" outline.="" forward.invert="function(x," y)="" (abs(y="" epsilon)="" [sinphi0="" atan(sinphi0="" x="" x,="" 0];="" k="x" y="" y,="" delta;="" do="" tanphi="tan(phi)," secphi="1" cos(phi),="" j="k" phi;="" =="" (tanphi="" (phi="" y))="" (2="" tanphi);="" (abs(delta)=""> epsilon && --i > 0);
